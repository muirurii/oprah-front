      ...
      <div className="mt-4 flex items-start justify-center gap-4 relative">
        <div className="max-w-2xl border shadow-sm shadow-gray-100 rounded-md p-4 pb-6">
          ...
          <div className="flex justify-between items-center my-4">
            <p className="text-sm text-gray-400">
              ...
              <span className="inline-block pl-4">{new Date(post.createdAt).toLocaleDateString()}</span>
            </p>
          ...
          <img src="/pic.jpg" className="w-full h-[400px] rounded-md" alt={post.title.slice(0, 6)} />
          ...
          <div className="min-h-[100px]">
              ...
              <div className="p-4 flex flex-col gap-4">
                 {post.comments.length ? post.comments.map((comm) => (
        ...
        <div className="w-1/3 shadow-sm shadow-gray-200">
          ...
          <div className="flex flex-col">
            {recommended.filter(p=> p._id !== post._id).map(post => <MoreCard key={post._id} post={post}/>)}
          </div>
        </div>
      </div>
    </div>
...
export const getStaticProps = async (context) => {
  const res = await fetch(
    `http://localhost:5000/api/posts/${context.params.slug}`
  );
